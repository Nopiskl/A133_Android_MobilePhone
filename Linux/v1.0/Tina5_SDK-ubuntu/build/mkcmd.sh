...
#'linux-4.9	arm64	gcc-linaro-5.3.1-2016.05-x86_64_aarch64-linux-gnu.tar.xz	rootfs_ubuntu_base_1604lts.tar.bz2'
'linux-4.9	arm64	gcc-linaro-5.3.1-2016.05-x86_64_aarch64-linux-gnu.tar.xz	rootfs_ubuntu_nopiskl_k5_1604lts.tar.gz'
)
...
function prepare_mkkernel()
{
	...
	if [ -n ${DTS_NAME} ]; then
		local board_dts="$LICHEE_BOARD_CONFIG_DIR/${DTS_NAME}.dts"
	else
...
function mk_defroot()
{
	...
	mk_info "Build default rootfs ... from $rootfs_archivedir "
	#if [ "$1" = "f" ]; then
		sudo rm -fr ${ROOTFS}
	#fi
	...
	if [ ! -d ${ROOTFS} ]; then
		...
		if [ -f ${rootfs_archivedir} ];then
			filename=${rootfs_archivedir}
			if [[ $filename == *.gz ]]; then
				echo "This is a gz file"
				#fakeroot tar -zxf ${rootfs_archivedir} -C ${ROOTFS}
				sudo tar -zxf ${rootfs_archivedir} -C ${ROOTFS}
			elif [[ $filename == *.bz2 ]]; then
				echo "This is a bz2 file"
				#fakeroot tar -jxf ${rootfs_archivedir} -C ${ROOTFS}
				sudo tar -jxf ${rootfs_archivedir} -C ${ROOTFS}
			else
				echo "$filename is neither a gz nor a bz2 file"
			fi
		else
	...
	#cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* \
	#	${ROOTFS}/lib/modules/
	(cd ${LICHEE_KERN_DIR}; sudo make ARCH=arm64 modules_install INSTALL_MOD_PATH=${ROOTFS})

	if [ "x$PACK_STABILITY" = "xtrue" -a -d ${LICHEE_KERN_DIR}/tools/sunxi ];then
		sudo cp -v ${LICHEE_KERN_DIR}/tools/sunxi/* ${ROOTFS}/bin
	fi
	if [ "x$LICHEE_LINUX_DEV" = "xsata" ];then
		if [ -d ${ROOTFS}/target ];then
			sudo rm -rf ${ROOTFS}/target
		fi
		if [ -d ${LICHEE_SATA_DIR}/linux/target ];then
			mk_info "copy SATA rootfs"
			sudo mkdir -p ${ROOTFS}/target
			sudo cp -a ${LICHEE_SATA_DIR}/linux/target/* ${ROOTFS}/target
		fi
	fi

	# overlay
	sudo cp -rf ${LICHEE_TOP_DIR}/overlay/* ${ROOTFS}/
	...
	bash ${LICHEE_BUILD_DIR}/mk-image.sh ${ROOTFS}
	################### change the param "-c" above the size of the rootfs.
	#fakeroot mkfs.ubifs -m 4096 -e 258048 -c 8192 -F -x zlib -r ${ROOTFS} -o ${LICHEE_PLAT_OUT}/rootfs.ubifs
	sudo mkfs.ubifs -m 4096 -e 258048 -c 8192 -F -x zlib -r ${ROOTFS} -o ${LICHEE_PLAT_OUT}/rootfs.ubifs
...
sudo ${LICHEE_BUILD_DIR}/bin/mksquashfs \
